...
export class JwtStrategy extends PassportStrategy(Strategy) {
  ...
  async validate(req: any, payload: any) {
    ...
    if (typeof payload === 'string') {
      console.error(
        'CRITICAL: JwtStrategy received a string payload instead of an object:',
    ...
    if (!profile) {
      console.warn(
        `Profile not found for user ${userId}. potentially blocked by RLS or user missing.`,
      );
    ...
    const permissions = ((permissionsData as any[]) || []).map(
      (p) => p.permission_name,
    );
