      ...
      const parseParts = (payload: any) => {
        ...
        if (isAttachment && payload.body?.attachmentId) {
          attachments.push({
            ...
            contentId: payload.headers?.find(
  ...
  async sendQuote(
    ...
    try {
      ...
      if (listRes.data.messages && listRes.data.messages.length > 0) {
        ...
        const existingMessageId = headers?.find(
          (h) => h.name?.toLowerCase() === 'message-id',
        )?.value;
    ...
    if (inReplyTo)
      emailLines.push(`In-Reply-To: ${this.sanitizeHeader(inReplyTo)}`);
    if (references)
      emailLines.push(`References: ${this.sanitizeHeader(references)}`);
