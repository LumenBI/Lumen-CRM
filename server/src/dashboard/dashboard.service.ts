...
export class DashboardService {
  ...
  async createAppointment(token: string, userId: string, payload: any) {
    ...
    const agentName =
      (Array.isArray(data.agent) ? data.agent[0] : data.agent)?.full_name ||
      'Un agente';
  ...
  async updateAppointment(token: string, id: string, payload: any) {
    ...
    if (currentParticipants && currentParticipants.length > 0) {
      ...
      console.warn(
        `[NOTIFY_FLOW] No participants found for appointment ${id}.`,
      );
    }
  ...
  async updateClient(token: string, id: string, payload: any) {
    ...
    if (payload.assigned_agent_id !== undefined) {
      ...
      if (authError)
        console.error('Error fetching user for reassignment check:', authError);
      ...
      if (user) {
        ...
        if (profileError)
          console.error(
            'Error fetching profile for reassignment check:',
            profileError,
          );
  ...
  async createDeal(token: string, userId: string, payload: any) {
    ...
    await this.createNotification(
      ...
      userId,
      'DEAL_CREATED',
      `Nueva negociación: ${payload.title}`,
      '/dashboard/kanban',
    );
    await this.notifyManagers(
      supabase,
      'DEAL_CREATED',
      `[${agent?.full_name || 'Agente'}] Nueva negociación: ${payload.title}`,
      '/dashboard/kanban',
    );
