    ...
    if (loading) {
        return (
            <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm h-full">
                <h3 className="mb-4 font-bold text-[#000D42]">{TEXTS.RECENT_ACTIVITY}</h3>
    ...
    return (
        <div className="rounded-xl border border-gray-100 bg-white p-6 shadow-sm h-full">
            ...
            {activitiesToShow.length === 0 ? (
                <div className="flex items-center justify-center h-32 border border-dashed border-gray-100 rounded-lg bg-gray-50/50">
                ...
                <div className="space-y-4">
                    {activitiesToShow.map((activity: any, idx) => (
                        <div key={activity.id || idx} className="flex gap-3 items-start group">
                            <div className="mt-1 p-2 rounded-full border border-slate-50 bg-slate-50 group-hover:bg-white group-hover:shadow-sm transition-all">
                                {getIcon(activity.category)}
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-[#000D42] truncate">
