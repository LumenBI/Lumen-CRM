...
export default function KpiCard({ title, value, change, trend, icon, color }: KpiCardProps) {
    return (
        <div className="group relative overflow-hidden rounded-2xl bg-white dark:bg-slate-900 p-6 shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 dark:border-slate-800 hover:scale-105">
            <div className={`absolute top-0 right-0 w-32 h-32 ${color} opacity-10 dark:opacity-20 rounded-full blur-3xl -mr-16 -mt-16 group-hover:opacity-20 dark:group-hover:opacity-30 transition-opacity`}></div>
            ...
            <div className="relative z-10">
                ...
                <div className="flex items-baseline gap-3 mb-2">
                    <p className="text-4xl font-bold text-[#000D42] dark:text-white">{value}</p>
                    {change && trend && (
